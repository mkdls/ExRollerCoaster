...
import erc._mc._1_7_10.item.itemSUSHI;
...
//            if (!worldObj.isRemote)
	...
	public void onUpdate()
	{
		syncToClient();
		if(seat == null) return;
		seat.GetParent().Update(worldObj.getWorldTime());
		savePrevData();
...
//        seat.Update(worldObj.getWorldTime());
	...
	protected void syncToClient()
	{
		if(this.UpdatePacketCounter--<=0)
		{
			...
			{
				Logger.debugInfo("cant find parent");
				setDead();
			}
			else DeadCounter = 10;
...
//                ERC_PacketHandler.INSTANCE.sendToAll(packet);
			...
			else
			{
				...
				Logger.debugInfo("sync client complete sync seat");
			}
		}
	}
